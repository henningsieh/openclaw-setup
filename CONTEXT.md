# OpenClaw Instance Context — Shelldon Native Setup

This is the full reference for the native Shelldon OpenClaw instance. `AGENTS.md`
is the short, always-loaded routing and safety guide; reach this document when a
task enters one of the branches listed there. Keep this file as the detailed
source of truth rather than duplicating it in `AGENTS.md`.
...
Keep live credentials and runtime state out of Git. Before pushing, review `git status` and the staged diff from `/home/shelldon/.openclaw`; personal data and secrets must not enter the repository.
