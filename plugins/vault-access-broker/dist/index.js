...
const unprovisionedCli = {
    async fetchLogin() {
        throw new Error("Vault Access Broker is not provisioned. Configure the pinned Vault Access Runtime before enabling this tool.");
    },
};
export function createVaultAccessBrokerPlugin(cli = unprovisionedCli) {
    return defineToolPlugin({
        ...
        tools: (tool) => [
            tool({
                name: "vault_fetch",
                label: "Vault Fetch",
                description: "Retrieve an approved username-and-password login pair for downstream use. Never repeat the response in chat.",
                optional: true,
